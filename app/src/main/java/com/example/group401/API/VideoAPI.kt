...
class VideoAPI {
    ...
    fun fetchRandomVideoDataOfCategory(count: Int,category: String): List<VideoItem> {
        val apiUrl = "http://181.215.69.116:7777/video/random/?category=$category&number=$count"
        // Send HTTP GET request and read the response
        val url = URL(apiUrl)
        val connection = url.openConnection() as HttpURLConnection
        connection.requestMethod = "GET"
